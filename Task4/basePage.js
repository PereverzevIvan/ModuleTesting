...
class BasePage {
    ...
    async isDisplayed(locator) {
        let needElementIsStillThere = false
        try {
            needElementIsStillThere = await driver.findElement(locator).isDisplayed()
        } catch {
            needElementIsStillThere = false
        }

        return needElementIsStillThere
    }
    ...
    async saveScreenshot(fileName) {
        driver.takeScreenshot().then(function(image) {
            require('fs').writeFileSync("./images/task4/" + fileName, image, 'base64')
        })
    }
